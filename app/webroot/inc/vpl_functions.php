...
// if $selected=1, that means this list is used in an edit
// form, and $selectVal is the selected value
function get_hours_selectlist($selectVal) {
	...
	for ($i=1; $i<13; $i++) {
		...
		$list .= "<option value='";
		$list .= $hrStr . "'";
if($selectVal == $hrStr) $list .= " selected";
		$list .= ">" . $hrStr . "</option>";
	}
	$list .= "</select>";
	return $list;
...
function get_minutes_selectlist($selectVal) {
	...
	for ($i=0; $i<60; $i++) {
		...
		$list .= "<option value='";
		$list .= $minStr . "'";
if($selectVal == $minStr) $list .= " selected";
		$list .= ">" . $minStr . "</option>";
	}
	$list .= "</select>";
	return $list;
...
<?php
	...
	$dateTimeStr = "";
	$len = count($dateTimeArray);
$total = $dateTimeArray[0]['NumOfSpace'];
$available = $total;

	for ($i=0; $i<$len; $i++) {
		//echo $dateTimeArray[$i]['EventID'];
		$eventID = $dateTimeArray[$i]['EventID'];
...
if ((isset($dateTimeArray[$i]['StartHour'])&& ($dateTimeArray[$i]['StartHour']!=0)) || ($dateTimeArray[$i]['ByApp'])) {
		...
		if (isset($dateTimeArray[$i]['DayOfWeek']) && ($dateTimeArray[$i]['DayOfWeek']!="")) {
			...
			if (($dateTimeArray[$i]['StartDate'] != $dateTimeArray[$i]['EndDate']) || (($dateTimeArray[$i]['StartMon_Num']==0) && ($dateTimeArray[$i]['EndMon_Num']==0))){
				...
				//echo sizeof($dates);
				for($i=0; $i < sizeof($dates); $i++){
					$occupied = countEventSpot($eventID, $dates[$i]);
					//echo "--".$total;
		...
		?>
				...
				<input type='radio' name='regDate' style='background-color: #000000 !important;' value='<?php echo $dates[$i]; ?>' <?php if($available ==0) echo 'DISABLED '; ?> > <?php echo $dates[$i] . "&nbsp;&nbsp;&nbsp;" . $temp_str; ?><?php echo "&nbsp;&nbsp;&nbsp;&nbsp;<i>(". $available . " spots available)</i>"; ?><br/>
		<?php
				}
			}
		...
		if (isset($dateTimeArray[$i]['StartDate']) && ($dateTimeArray[$i]['StartDOM']!=0)) {
			$occupied = countEventSpot($eventID, $dateTimeArray[$i]["StartDate"]);
			$available = $total - $occupied;
?>

				<input type='radio' name='regDate' style='background-color: #000000 !important;' value='<?php echo $dateTimeArray[$i]["StartDate"]; ?>'><?php echo $dateTimeArray[$i]["StartDate"]  . "&nbsp;&nbsp;&nbsp;" . $temp_str; ?><?php echo "&nbsp;&nbsp;&nbsp;&nbsp;<i>(". $available  . " spots available)</i>"; ?><br/>
		<?php
			//echo "<option value=\"" . $dateTimeArray[$i]['StartDate']. "\">". $dateTimeArray[$i]['StartDate'] . "</option>";
		}
...
?>
<input type="hidden" name="timeStr" value="<?php echo $temp_str; ?>">
...
function get_next_date($dateTimeArray)
{
?>
...
$today = mktime(0,0,0,9,6,2013);
$timeSlot = array('eventDate'=>'', 'timeStr'=> '', 'eventID'=> '');
$flag=0;
...
if ((isset($dateTimeArray[$i]['StartHour'])&& ($dateTimeArray[$i]['StartHour']!=0))) {

			if ($temp_str!="") $temp_str .= ",&nbsp;";

			if (isset($dateTimeArray[$i]['StartMin'])) {
		...
		if (isset($dateTimeArray[$i]['DayOfWeek']) && ($dateTimeArray[$i]['DayOfWeek']!="")) {
			...
			if (($dateTimeArray[$i]['StartDate'] != $dateTimeArray[$i]['EndDate']) || (($dateTimeArray[$i]['StartMon_Num']==0) && ($dateTimeArray[$i]['EndMon_Num']==0))){
				...
				for($i=0; $i < sizeof($dates); $i++){
					...
					$day_diff = round((strtotime($dates[$i])-$today)/86400);

//echo $day_diff."--";

if( $day_diff >= 0 && $flag == 0 ){
		 echo $dates[$i] . "&nbsp;&nbsp;&nbsp;" . $temp_str . "&nbsp;&nbsp;&nbsp;&nbsp;<i>(". $available . " spots available)</i><br/>";

		 $timeSlot = array('eventDate'=>$dates[$i], 'timeStr'=> $temp_str, 'eventID'=> $eventID);
		$flag = 1;
}
				}
			}
...
$day_diff = round((strtotime($dateTimeArray[$i]["StartDate"])-$today)/86400);

if( $day_diff >= 0 && $flag == 0  ){
			echo $dateTimeArray[$i]["StartDate"]  . "&nbsp;&nbsp;&nbsp;" . $temp_str; ?><?php echo "&nbsp;&nbsp;&nbsp;&nbsp;<i>(". $available  . " spots available)</i><br/>";
			$timeSlot = array('eventDate'=>$dateTimeArray[$i]['StartDate'], 'timeStr'=> $temp_str, 'eventID'=> $eventID);
$flag = 1;
}
		}
...
function print_eproduct_kidzone($productid) {
	...
	<a name="<?php echo $eproduct['ShortName']; ?>"></a>
	<a href="<?php echo $eproduct['URL']; ?>" class="homeworkzone-content-link" target="_blank" alt="<?php echo $eproduct['Name']; ?>"><?php echo $eproduct['Name']; ?></a>
