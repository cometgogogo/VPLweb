...
function get_all_public_branches() {
	$result = mysql_query("SELECT * FROM Branches WHERE BranchID<>4 AND BranchID<>9 ORDER BY BranchShort");
	return $result;
}
...
/*
 * search_atl_events_by_program_schedule_branch:
 *
 * Search in ATL database for all the events by
 * program id and schedule id.
 */
function search_atl_events_by_program_schedule_branch($program, $schedule, $branch)
{
	$result = mysql_query("SELECT DISTINCT ATL_Events.EventID, Branches.BranchName FROM ATL_Events, Branches WHERE ATL_Events.PublicityStatus=1 AND ATL_Events.ProgramID='$program' AND ATL_Events.BranchID=Branches.BranchID AND ATL_Events.ScheduledFor='$schedule' AND Branches.BranchID = '$branch'");
...
function search_atl_eventtime_by_eventid_ming($eventid) {
	$result = mysql_query("SELECT StartDate, EndDate, DayOfWeek, DAYOFMONTH(StartDate) AS StartDayOfMonth, DAYOFMONTH(EndDate) AS EndDayOfMonth, YEAR(StartDate) AS StartYear, YEAR(EndDate) AS EndYear, MONTHNAME(StartDate) AS StartMonth, MONTH(StartDate) AS StartMon_Num, MONTHNAME(EndDate) AS EndMonth, MONTH(EndDate) AS EndMon_Num, HOUR(ATL_Event_Time.StartTime) AS StartHour, MINUTE(ATL_Event_Time.StartTime) AS StartMinute, HOUR(ATL_Event_Time.EndTime) AS EndHour, MINUTE(ATL_Event_Time.EndTime) AS EndMinute, ByAppointment, ATL_Events.NumOfSpace AS NumOfSpace, ATL_Events.EventID AS EventID FROM ATL_Event_Time, ATL_Events WHERE ATL_Event_Time.EventID='$eventid' AND ATL_Event_Time.EventID=ATL_Events.EventID");

	return $result;
}

function search_atl_eventtime_ming($eventid) {

    $sql = "SELECT * FROM ATL_Event_Reg WHERE EventID='$eventid'";
	$result = mysql_query($sql);

	return $result;
}
//--------TLV begins---
function searchSignedVolunteerByEmail($email, $eventID, $eventDate){
	   $sql = "SELECT * FROM ATL_Reg_Info WHERE EventID='$eventID' AND EventDate='$eventDate' AND Email='$email'";
	   //echo $sql;
		$result = mysql_query($sql);
	return $result;
}

function searchVolunteerByEmail($email){
	   $sql = "SELECT * FROM TLV_info WHERE Email='$email'";
	//echo $sql;
		$result = mysql_query($sql);
	return $result;
}

function countEventSpot($eventID, $eventDate){
	$sql = "SELECT * FROM ATL_Reg_Info WHERE EventID='$eventID' AND EventDate='$eventDate'";
	//echo $sql;
...
/*
 * search_src_by_branch_schedule_keywords:
 *
 * Search in ATL database for the live special
 * programs by branch and schedule for the given keyword.
 */
function search_src_by_branch_schedule_keywords($branch, $schedule, $keyword) {
	$result = mysql_query("SELECT ATL_Events.EventID, Branches.BranchName, ATL_Programs.ProgramID, ATL_Programs.ProgramName FROM Branches, ATL_Programs, ATL_Events, ATL_Event_Time WHERE ATL_Events.PublicityStatus=1 AND ATL_Events.ScheduledFor='$schedule' AND ATL_Programs.Keywords LIKE '%$keyword%' AND ATL_Programs.ProgramID=ATL_Events.ProgramID AND ATL_Events.EventID=ATL_Event_Time.EventID AND ATL_Events.BranchID='$branch' AND ATL_Events.BranchID=Branches.BranchID ORDER BY ATL_Event_Time.StartDate");
