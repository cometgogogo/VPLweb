...
<?php $html->addCrumb("About Vaughan Public Libraries" , "/about"); ?>
<?php $html->addCrumb("Locations and Hours" , "/libraries"); ?>
	...
	<div class="details">
		...
		<?php } else { ?>
			<div class="intro">
				...
				<?php } ?>
				<?php if (!$details["elibrary"]) { ?>
					<a class="map_link" href="#direction">Direction</a>&nbsp;&nbsp;&nbsp;<a class="map_link" href="/libraries/map">Map</a>
...
<!--		
		...
		<a href="/about/notice">The Library will be closed for renovations from Friday, September 24 to Monday, October 11.</a><br/><br/>			
	</p>
<?php //} ?>
-->		
...
<?php if ($library['Library']['BranchID'] == "8") { ?>
	...
	<p class="notice_link">
	<b>Notice: </b>
	Due to network problems, access to the Internet is temporarily unavailable on Woodbridge library computers. Staff are also unable to check materials in, or to update customers library records. We hope to resume service as soon as possible. We apologize for the inconvenience.
	 <br/><br/>			
	</p>
<?php } ?>
-->
		...
		<!--<img class="img_wrapped_right" width="250px" src="/img/myCalendar.gif" title="<?php echo 'Check out What\'s On @ ' . $library['Library']['BranchName']; ?>" / -->
		<?php if (count($details["hours"]) > 0) { ?>
			<div class="time_table">
				...
				<?php if ($library['Library']['BranchID'] != "2" && $library['Library']['BranchID'] != "7") { ?>
					<div class="note">* The library is closed on Sundays in July and August.</div>
				<?php } ?>
				<!--
				<?php if ($library['Library']['BranchID'] == "2" || $library['Library']['BranchID'] == "7") { ?>
					<div class="note">* Effective Monday, September 14, 2009, the library will be open to the public Monday mornings.</div>
				<?php } ?>
				-->
			</div>
		<?php } ?>
			...
			<div class="feature_chart">
				<ul>
					<?php foreach ($library["LibraryFeature"] as $libraryFeature) { ?>
						<li id="<?php echo $libraryFeature["Image"];?>"><?php echo $html->link($libraryFeature["Name"], "/library_features/view/" . $libraryFeature["BranchFeatureID"]); ?></li>
					<?php } ?>
		...
		<?php if ($details["multilingual"]) { ?>
			<div class="entry">
				...
				<p><?php echo $library['Library']['BranchName']?> has <?php echo $html->link("collections of materials","/multilingual_collection"); ?> in <?php  echo $details["multilingual"]; ?> other than English and French.
				</p>
			</div>
		<?php } ?>
		...
		<?php if ($location_js <> "") { ?>
			<div class="map_entry">
				<h2><a name="direction">Directions by Google Map</a></h2>
				<!--script src="http://www.gmodules.com/ig/ifr?url=http://hosting.gmodules.com/ig/gadgets/file/114281111391296844949/driving-directions.xml&amp;up_myLocations=<?php echo $location_js; ?>&amp;synd=open&amp;w=400&amp;h=69&amp;title=&amp;border=%23ffffff%7C3px%2C1px+solid+%23999999&amp;output=js"></script-->
				
				<script src="http://www.gmodules.com/ig/ifr?url=http://hosting.gmodules.com/ig/gadgets/file/114281111391296844949/driving-directions.xml&amp;up_fromLocation=&amp;up_myLocations=<?php echo $location_js; ?>&amp;up_defaultDirectionsType=&amp;up_autoExpand=&amp;synd=open&amp;w=320&amp;h=800&amp;brand=light&amp;lang=en&amp;country=US&amp;output=js"></script>
		...
		<?php if ($details["elibrary"]) { ?>
			<div class="entry">
				...
				<p>
					If you are not already a Vaughan Public Libraries member, <?php echo $html->link("apply for membership online","#"); ?>.
				</p>
			</div>
			<div class="entry">
				<h2>Search the Vaughan Public Libraries Catalogue</h2>
				<p>
					Browse the <?php echo $html->link("library catalogue","/materials/catalogues"); ?>library catalogue, reserve, and renew library materials.
				</p>
			</div>
			<div class="entry">
				...
				<p>
					...
					questions to <?php echo $html->link("Email Librarian","/email_librarian"); ?> and your question will be answered within
					24 hours.
				</p>
			</div>
			<div class="entry">
				...
				<p>
					<?php echo $html->link("Librarian-researched Web sites","/links"); ?> for general information, for study or for business.
				</p>
			</div>
			<div class="entry">
				<h2>Search Premium Databases</h2>
				<p>
					Vaughan Public Libraries subscribe to a number of <?php echo $html->link("databases","/databases"); ?> that are free to VPL members.
				</p>
			</div>
			<div class="entry">
				<h2>Find Information About Vaughan</h2>
				<p>
					<?php echo $html->link("eVaughan","/community/vaughan"); ?> is a growing portal to information for everyday life in the City of Vaughan.
				</p>
			</div>
			<div class="entry">
				<h2>Check what's On</h2>
				<p>
					Search for library <?php echo $html->link("programs","/programs"); ?> of interest to VPL members of all ages.
				</p>
			</div>
			<div class="entry">
				...
				<p>
					Use
					our <?php echo $html->link("form","/user_requests/add/purchase"); ?> to suggest an item you would like added to our collection.
			...
			<div class="entry">
				...
				<p>
					Find out what's on at your local <?php echo $html->link("library","/libraries"); ?>, call library staff with your reference or membership questions and more.
				</p>
			</div>
		<?php } ?>
		...
		<?php if (count($details["pictures"]) > 0) { ?>
			<div id="photo_tour">
				...
				<noscript>
					...
					<?php
						if ($currentPicture > 0) {
							echo $html->link("Previous","/libraries/view/" . $library['Library']['BranchID'] . "/" . $currentPicture . "/#photo_tour") . "&nbsp;&nbsp;";
						} else {
							echo "Previous&nbsp;&nbsp;";
						}
						foreach ($details["pictures"] as $element => $picture) {
							if ($element == $currentPicture) {
								...
								echo $html->link($element+1, "/libraries/view/" . $library['Library']['BranchID'] . "/" .  ($element+1) . "/#photo_tour") . "&nbsp;&nbsp;";
							}
						}
						if ($currentPicture+1 < count($details["pictures"])) {
							echo $html->link("Next","/libraries/view/" . $library['Library']['BranchID'] . "/" . ($currentPicture +2) . "/#photo_tour");
						} else {
							echo "Next&nbsp;&nbsp;";
						}
				...
				<div id="control_javascript">
					...
					<?php
					echo $html->link("Previous","javascript:onPicturePrevious()"). "&nbsp;&nbsp;";
					foreach ($details["pictures"] as $element => $picture) {
						echo $html->link($element+1, "javascript:onPictureChange(" . ($element+1) . ");", array("id"=>"picture_link_" . $element, "class"=> ($element == $currentPicture ? "current" : ""))) . "&nbsp;&nbsp;";
					}
					echo $html->link("Next","javascript:onPictureNext()");
					?>
				</div>
			</div>&nbsp;
		<?php } ?>
