...
class SkitterHelper extends AppHelper
{
    ...
    function includeFiles() {
		...
		$out .= $this->Html->script('jquery.animate-colors-min');
		$out .= $this->Html->script('jquery.easing.1.3');
		$out .= $this->Html->script('jquery.skitter.min');
	...
	public function display ($slides = array()) {
		if (!empty($slides)) {
			...
			foreach($slides as $slide) {
				...
				$title = (isset($slide['label']) ? $slide['label'] : '');
				//$image = $this->Html->image($slide['image'], array('class' => $animation));
				$image = '<img src="/img/'.$slide[image].'" class="'.$animation.'" alt="'. $title.'" />';

				$label = null;

				if (!empty($slide['link'])) {
					//$image = $this->Html->link($image, $slide['link'], array('target'=>'_blank', 'escape' => false));
					$image = '<a href="'.$slide['link']. '"  rel="external" target="_blank">'.$image.'</a>';
				}
