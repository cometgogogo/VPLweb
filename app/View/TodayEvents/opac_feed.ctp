					...
					switch ($currentScheduleType) {
						case "date":
							...
							if ($event['Event']['date'] == $currentDate) { 							
								...
								$display = "<item><title>".utf8_encode($event['Program']['name'])." --- ";
								$event_desc = "";
								...
								echo nl2br($display) . "</title><description>event</description></item>";	
							}
						...
						case "period":
							...
							foreach($event['EventSlots'] as $slot){	
								...
								if($slot['day_of_week'] == date('l')){	
									...
									$display .= " " .$event_desc. " ";
									echo "  ".nl2br($display)."</title><description>Events</description></item>";								
