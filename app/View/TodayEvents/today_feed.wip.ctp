	...
	<channel>
		...
		<width>35</width>
			...
			<item>
				...
				echo "<guid>http://www.vaughanpl.info/programs/view/" . $event['Program']['id'] . "</guid>";
				...
				foreach($ongoing['EventSlots'] as $slot){	
					...
					if($slot['day_of_week'] == date('l')){								
						...
						if($ongoing['Program']['name'] != $temp){
							$display .= "<item>";
							$flag=1;
							$display .= "<title><![CDATA[". utf8_encode($ongoing['Program']['name']) . "]]> </title>"; 
							$display .= "<link>http://www.vaughanpl.info/programs/view/" . $ongoing['Program']['id'] . "</link>";
							$display .= "<guid>http://www.vaughanpl.info/programs/view/" . $ongoing['Program']['id'] . "</guid>";
							$temp = $ongoing['Program']['name'];
							$count ++;
							$display .= "<description>";
							$display .= utf8_encode($ongoing['Program']['description']);	
							$display .= "\n";
						}
